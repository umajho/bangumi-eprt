import { type Component, createMemo, Show } from "solid-js";
import { customElement, noShadowDOM } from "solid-element";

import {
  type EpisodeData,
  type EpisodeId,
  makeCustomElementTagName,
  type Score,
  type SubjectId,
} from "../definitions";
import * as epDataHelpers from "../utils/episode-data-helpers";
import { readonlyPageData } from "../stores/readonly-page-data";
import { SmallStars } from "./SmallStars";
import type { Context } from "../context";
import { getMyRatingModalSingletonInstance } from "./modals/MyRatingModal";

const TAG_NAME = makeCustomElementTagName("my-rating-compact");

export function createMyRatingCompactInstance(
  ctx: Context,
  opts: { subjectId: SubjectId; episodeId: EpisodeId },
) {
  registerMyRatingCompact(ctx);
  const el = document.createElement(TAG_NAME);
  el.setAttribute("subject-id", String(opts.subjectId));
  el.setAttribute("episode-id", String(opts.episodeId));

  return { element: el };
}

let elementConstructor: CustomElementConstructor | null = null;

function registerMyRatingCompact(ctx: Context) {
  elementConstructor ??= customElement(
    TAG_NAME,
    {
      subjectId: null,
      episodeId: null,
    },
    (props) => {
      noShadowDOM();

      return (
        <Show
          when={Number.isInteger(props.subjectId) &&
            Number.isInteger(props.episodeId)}
        >
          <MyRatingCompact
            prefersFetchingCompleteSubjectVotes={false}
            ctx={ctx}
            subjectId={props.subjectId!}
            episodeId={props.episodeId!}
          />
        </Show>
      );
    },
  );
}

export const MyRatingCompact: Component<{
  label?: string;
  prefersFetchingCompleteSubjectVotes: boolean;
  ctx: Context;
  subjectId: SubjectId;
  episodeId: EpisodeId;
  noVisibilityHint?: boolean;
}> = (props) => {
  const dataResp = props.ctx.scoreStore.queryEpisodeDataTracked(
    props.subjectId,
    props.episodeId,
    {
      prefersFetchingCompleteSubjectVotes:
        props.prefersFetchingCompleteSubjectVotes,
    },
  );

  const data = epDataHelpers.createData(dataResp);

  return (
    <Show when={data()}>
      {(episodeData) => (
        <>
          {props.label}
          <MyRatingCompactInner
            prefersFetchingCompleteSubjectVotes={props
              .prefersFetchingCompleteSubjectVotes}
            ctx={props.ctx}
            subjectId={props.subjectId}
            episodeId={props.episodeId}
            shouldShowVisibilityHint={!props.noVisibilityHint}
            data={episodeData()}
          />
        </>
      )}
    </Show>
  );
};

const MyRatingCompactInner: Component<{
  prefersFetchingCompleteSubjectVotes: boolean;
  ctx: Context;
  subjectId: SubjectId;
  episodeId: EpisodeId;
  shouldShowVisibilityHint: boolean;
  data: EpisodeData;
}> = (props) => {
  const scoreAndHint = createMemo(
    (): {
      score: Score | null;
      hintType: "public" | "private" | "unknown" | null; // 实际上不应该进入 `unknown` 状态。
    } | null => {
      if (props.data.myRating) {
        if (props.data.myRating.score === null) {
          return { score: null, hintType: null };
        }
        return {
          score: props.data.myRating.score,
          hintType: ((v) => {
            if (v === "unknown") return "unknown";
            return v.isVisible ? "public" : "private";
          })(props.data.myRating.visibility),
        };
      }
      if (!readonlyPageData.claimedUserId) return null;
      if (!props.data.publicVotersByScore) return null;
      for (
        const [scoreStr, voters] of Object.entries(
          props.data.publicVotersByScore,
        )
      ) {
        if (voters.some((voter) => voter === readonlyPageData.claimedUserId)) {
          return { score: Number(scoreStr) as Score, hintType: "public" };
        }
      }
      return null;
    },
  );

  return (
    <Show when={scoreAndHint()}>
      {(scoreAndHint) => (
        <>
          <SmallStars score={scoreAndHint().score} shouldShowNumber={false} />
          <Show
            when={props.shouldShowVisibilityHint && scoreAndHint().hintType}
            fallback={
              <ShowModalButton
                prefersFetchingCompleteSubjectVotes={props
                  .prefersFetchingCompleteSubjectVotes}
                ctx={props.ctx}
                subjectId={props.subjectId}
                episodeId={props.episodeId}
              />
            }
          >
            <span class="tip_j">
              (<ShowModalButton
                prefersFetchingCompleteSubjectVotes={props
                  .prefersFetchingCompleteSubjectVotes}
                ctx={props.ctx}
                subjectId={props.subjectId}
                episodeId={props.episodeId}
              />
              {((type) => {
                switch (type) {
                  case "public":
                    return "公开";
                  case "private":
                    return "不公开";
                  case "unknown":
                    return "可见性未知";
                }
              })(scoreAndHint().hintType)}
              )
            </span>
          </Show>
          {" "}
        </>
      )}
    </Show>
  );
};

const ShowModalButton: Component<{
  prefersFetchingCompleteSubjectVotes: boolean;

  ctx: Context;

  subjectId: SubjectId;
  episodeId: EpisodeId;
}> = (props) => {
  const singleton = getMyRatingModalSingletonInstance(props.ctx);

  return (
    <button
      style={{ "font-size": "12px", padding: 0 }}
      onClick={() =>
        singleton.open({
          prefersFetchingCompleteSubjectVotes:
            props.prefersFetchingCompleteSubjectVotes,
          subjectId: props.subjectId,
          episodeId: props.episodeId,
        })}
    >
      评分
    </button>
  );
};
